# --- 第一阶段：构建阶段 ---
    ...
    RUN uv run python -c "from demucs.pretrained import get_model; get_model('htdemucs')" || true
    RUN uv run python -c "from speechbrain.inference.speaker import EncoderClassifier; from speechbrain.utils.fetching import LocalStrategy; EncoderClassifier.from_hparams(source='speechbrain/spkrec-ecapa-voxceleb', savedir='pretrained_models/spkrec-ecapa-voxceleb', local_strategy=LocalStrategy.COPY)" || true
